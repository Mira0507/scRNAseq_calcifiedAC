...
res.df <- as.data.frame(res) %>% 
    ...
    mutate(Statistics=ifelse(padj > 0.05 | is.na(padj), 
                             "Insignificant", 
...
for (x in my.clusters) {
    ...
    gse <- gseGO(geneList=geneList, 
                 OrgDb=id.db,
                 keyType="SYMBOL", # Check out supported keytypes with keytypes(id.db)
                 ont="BP",     # "ALL", "CC", "MF", or "BP"
                 verbose=T, 
                 minGSSize=3,   # Min gene set size 
                 maxGGSize=800, # Max gene set size
                 pvalueCutoff=0.05, 
                 pAdjustMethod="BH",
